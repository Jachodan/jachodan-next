...
export default function StockControl({ currentStock, onStockChange, variant = "list" }: StockControlProps) {
    ...
    const handleActionConfirm = () => {
        ...
        if (!isNaN(amount) && amount > 0) {
            if (actionType === "in") {
                onStockChange(currentStock + amount);
            } else if (actionType === "out") {
                onStockChange(Math.max(0, currentStock - amount));
            }
        }
        setActionType(null);
        setActionValue("");
