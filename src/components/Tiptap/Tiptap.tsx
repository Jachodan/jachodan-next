...
const Tiptap = ({
    ...
    const cssVariables = {
        '--tiptap-command-filter': `'${TIP_TAP_LOCALE.COMMAND_FILTER}'`,
        '--tiptap-node-toggle-placeholder': `'${TIP_TAP_LOCALE.PLACEHOLDER_TOGGLE}'`,
                ...
                <DragHandle editor={editor} className="flex">
                    <Tooltip>
                        ...
                        <TooltipContent className="text-center">
                            <strong>{TIP_TAP_LOCALE.DRAG_HANDLE.WITH_DRAG}</strong> {TIP_TAP_LOCALE.DRAG_HANDLE.MOVE}
                        </TooltipContent>
                    </Tooltip>
                </DragHandle>
            )}
            <EditorContent id={id} editor={editor} className={className ?? "py-8 px-12"} />
        </div>
    );
};
