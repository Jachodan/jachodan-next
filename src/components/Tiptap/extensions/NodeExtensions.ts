...
import mentionSuggestion from '@/components/Tiptap/extensions/suggestions/mentionSuggestion';
...
export default function NodeExtensions({
    ...
    return [
        ...
        Placeholder.configure({
            placeholder: ({ node }) => {
                if (node.type.name === 'details') {
                    return TIP_TAP_LOCALE.PLACEHOLDER_TOGGLE;
                } else if (node.type.name === 'detailsContent') {
                    return TIP_TAP_LOCALE.PLACEHOLDER_CONTENT;
                } else if (node.type.name === 'paragraph') {
                    return TIP_TAP_LOCALE.PLACEHOLDER;
                } else if (
        ...
        ImageUploadNode.configure({
            ...
            onError: (error) => {
                toast.error(TIP_TAP_LOCALE.IMAGE_UPLOAD_NODE.IMAGE_UPLOAD_FAILED(error.message));
            },
